...
Raw source data are not included in this repository. Retrieve the required
water-level and meteorological records using the station information provided
in `data/README.md`.

For a complete reproduction from the source data, place the downloaded files
in the following local directories:
...
The analyses were conducted in Python using Jupyter Notebook.

Install the package versions listed in requirements.txt before running the notebooks.

### 3. Run the notebooks
For a complete reproduction of the analytical workflow, run the notebooks in the following order:
...
Notebook 01 performs the main data preprocessing and generates the processed dataset:

data/processed/water_climate_10_lakes_primary.csv

This processed dataset is subsequently used as the primary input for Notebooks 02–04.

Because the processed dataset is included in this repository, users interested primarily in reproducing the forecasting analyses may run Notebooks 02–04 without first reproducing the raw-data preprocessing in Notebook 01.

### 4. Generated outputs
Selected numerical results and figures generated by the notebooks are written to the outputs/ directory.

These outputs can be compared with the reference outputs supplied in the repository to verify successful reproduction of the reported analyses.


## Reference Outputs

The `outputs/` directory contains selected final analytical outputs corresponding to results reported in the dissertation.

These materials are provided to support verification of reproduced results. Intermediate debugging files, superseded model versions and analyses not reported in the dissertation are intentionally excluded.
...
Within the analytical code, "Lake Woods" is used as the internal label for
Lake of the Woods.
